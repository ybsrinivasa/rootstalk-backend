...
async def test_set_pv_blocks_when_sibling_has_same_fingerprint_and_shared_district(db):
    ...
    # PoP A: location D1, then fingerprint {param: v1}. Order matters
    # — if both PoPs were empty-fingerprint sharing D1, the empty-vs-
    # empty conflict would surface at the location save (covered by
    # `test_set_pv_blocks_both_empty_fingerprints_in_shared_district`).
    ...
    # PoP B picks up the same district (no conflict yet — B's PV is empty).
    await set_package_locations(
    ...
    # PoP B now tries the same {param: v1} — must be blocked.
    with pytest.raises(HTTPException) as ei:
        await set_package_variables(
            client_id=client.id, package_id=pkg_b.id,
            request=PackageVariableSet(assignments=[
...
async def test_set_pv_succeeds_when_fingerprint_differs(db):
    ...
    # Same staggering: A gets location + PV first, then B's location
    # (B still empty-PV but A is non-empty so no equality), then B's
    # PV with a DIFFERENT variable.
    await set_package_locations(
        client_id=client.id, package_id=pkg_a.id,
        locations=[PackageLocationIn(state_cosh_id="S1", district_cosh_id="D1")],
        db=db, current_user=user,
    ...
    # Differs at the param value — fine.
