...
async def test_qa_list_standard_responses_carries_timeline_count(db):
    ...
    db.add_all([
        PGTimeline(standard_response_id=sr.id, name="W1", from_value=0, to_value=7),
        PGTimeline(standard_response_id=sr.id, name="W2", from_value=7, to_value=14),
    ])
    await db.commit()
...
async def test_qa_timelines_excludes_pg_rooted_rows(db):
    ...
    db.add(sr); await db.flush()
    db.add(PGTimeline(standard_response_id=sr.id, name="QA-W1", from_value=0, to_value=7))
    ...
    db.add(PGTimeline(pg_recommendation_id=pg.id, name="PG-W1", from_value=0, to_value=7))
    await db.commit()
...
async def test_qa_timelines_chip_filter_by_sr(db):
    ...
    db.add_all([
        PGTimeline(standard_response_id=sr1.id, name="A", from_value=0, to_value=7),
        PGTimeline(standard_response_id=sr2.id, name="B", from_value=0, to_value=7),
    ])
    await db.commit()
...
async def test_qa_practices_breadcrumb(db):
    ...
    db.add(sr); await db.flush()
    tl = PGTimeline(standard_response_id=sr.id, name="W1", from_value=0, to_value=7)
...
async def test_qa_practices_excludes_pg_rooted(db):
    ...
    db.add(pg); await db.flush()
    pg_tl = PGTimeline(pg_recommendation_id=pg.id, name="PG-W1", from_value=0, to_value=7)
